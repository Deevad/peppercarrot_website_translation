<?php

$LANG = array(

# Global
'TRANSLATED_BY' 		    => 'Překlad: Kateřina Fleknová',
'LANGUAGE_NAME' 			=> 'Čeština',
'LANGUAGE_ISO_CODE_2_LETTER'=> 'cs',
# http://www.w3schools.com/tags/ref_language_codes.asp --> full list

################################################################################
# Header HTML infos for search engine and title in tab:
'PEPPERCARROT_VEGETABLE'    =>  'Feferonka a Karotka', // litteral translation of spices + vegetable
'Website_DESCRIPTION'        =>  'Oficiální stránky Pepper&amp;Carrot, bezplatného open-source komiksu o mladé čarodějce Feferonce a jejím kocourovi Karotkovi. Společně žijí ve světě plném kouzel, lektvarů a magických stvoření.',
'SUBTITLE'                  =>  'Open Webcomic vytvořil David Revoy',

################################################################################
# Top menu website:
'HOMEPAGE'      =>  'Domovská stránka',
'WEBCOMICS'     =>  'Komiks',
'BLOG'          =>  'Blog',
'PHILOSOPHY'    =>  'Pojetí',
'CONTRIBUTE'    =>  'Podílet se',
'SOURCES'       =>  'Zdroje',
'AUTHOR'        =>  'Autor',
'FOLLOW'        =>  'Sledujte Pepper&amp;Carrot na:',

################################################################################
# Top and bottom translation panel
'ADD_TRANSLATION'       => 'přidat překlad',
'CORRECTIONS' 			=> 'navrhnout opravu',

################################################################################
# Page: Homepage
'HOMEPAGE_BIG_TEXT'		    =>  '
    Zcela zdarma open-source komiks<br/>
    podporovaný přímo svými fanoušky<br/>
    ve snaze změnit komiksový průmysl!<br/>
',
'HOMEPAGE_PATREON_INFO'     =>  'Jen za 1$ podpořte vznik dalších Feferončiných dobrodružství',
'HOMEPAGE_MOREINFO_BUTTON'  =>  'Více informací',
'HOMEPAGE_PATREON_BUTTON'   =>  'Podpořte na Patreonu novou epizodu Pepper&amp;Carrot',
'HOMEPAGE_LAST_EPISODE'     =>  'Nejnovější epizoda:',
'HOMEPAGE_NEWS_UPDATE'      =>  'Novinky a aktualizace:',
'HOMEPAGE_MOREPOSTS_BUTTON' =>  'Další příspěvky',
'HOMEPAGE_MAINSERVICE_LINK' =>  'https://www.patreon.com/davidrevoy',

################################################################################
# Page: Webcomics
'WEBCOMIC_EPISODE'		    =>  'Epizody komiksu',
'WEBCOMIC_MAKINGOF'		    =>  'Za scénou',
'WEBCOMIC_MAKINGOF_DESCRIPTION' =>  '
    <p> Nahlédnutí za scénu a <a href="http://www.davidrevoy.com/categorie5/tutorials">návody</a> jsou speciálním bonusem, sponzorovaným mými fanoušky na <a href="https://www.patreon.com/davidrevoy">Patreonu</a></p>
',
'WEBCOMIC_ARTWORK'		    =>  'Galerie ilustrací',
'WEBCOMIC_SKETCHES'		    =>  'Skicy',

################################################################################
# Page: Blog
# when content are not English (no need to warn user the content is english only here):
#'LIMITATIONS' 	=> '<i class="fa fa-info-circle"></i> Content available in english only ',
'LIMITATIONS' 		=> 'Obsah dostupný pouze v angličtině',

################################################################################
# Page: Philosophy
'PHILOSOPHY_CONTENT'    =>  '
 <h2>Podporovaný fanoušky</h2>

    <p>Pepper&amp;Carrot je projekt sponzorovaný jen svými fanoušky z celého světa.<br/>
    Každý fanoušek na nově vydanou epizodu přispěje 1 dollar a za to bude jeho jméno napsáno v poděkování autora na konci nové epizody.<br/>
    Díky tomuto systému může Pepper&amp;Carrot zůstat nezávislým komiksem bez jakýkoliv otravných reklam.</p>

    <div class="philobutton">
        <a href="https://www.patreon.com/davidrevoy" title="Jen za 1$ na Patreonu podpořte novou epizodu Pepper&amp;Carrot">
            Jen za 1$ na epizodu přispějte ke vzniku nových Feferončiných a Karotkových dobrodružství.
        </a>
    </div>

        <a href="https://www.patreon.com/davidrevoy" title="Jen za 1$ na Patreonu podpořte novou epizodu Pepper&amp;Carrot">
            <img alt="illustration representing patronage" src="data/images/static/2015-02-09_philosophy_01-support.jpg">
        </a>

    <h2>Navždy 100% zdarma, žádný placený přístup</h2>

    <p>Všechen můj obsah vytvořený k Pepper&amp;Carrot je na těchto stránkách zdarma a všem volně k dostání.<br/>
    Všech z Vás si vážím stejně; ať už platíte nebo ne. Všechny speciální bonusy, které vytvářím pro přispívající fanoušky, zveřejňuji i tady na stránkách.<br/>
    S Pepper&amp;Carrot po Vás nikdy nebudeme chtít, abyste něco platili nebo si přihlásili předplatné, se kterým byste měli přístup k nejnověším epizodám.</p>

        <img alt="Illustration representing paywall" src="data/images/static/2015-02-09_philosophy_03-paywall.jpg">

    <h2>Open-source a liberální</h2>

    <p>Chci dát lidem možnost volně sdílet, používat, stavět a dokonce i na mém díle vydělávat.<br/>
    Všechny stránky, ilustrace a obsah byl vytvořen za pomoci bezplatných/Open-source programů na GNU/Linux a všechny zdroje jsou tady stránkách, stačí v menu kliknout na Zdroje.<br/>
    Komerční využití, překlady, tvorba fanoušků, tisk, filmy, videohry, sdílení a rozšiřování je podporováno.<br/>
    Stačí když přiznáte právoplatná práva autorům (umělci, korektoři, překladatelé zapojení do díla, které chcete použít), přiložíte odkaz na licenci a poukážete na provedené změny. To lze provést jakýmkoli rozumným způsobem, ledaže by sdělení naznačovalo podporu ze strany autora (autorů) Vám anebo Vašemu dílu.</p>

    <div class="philobutton">
        <a href="https://creativecommons.org/licenses/by/4.0/" title="Pro více informací si přečtěte Creative Commons Attribution 4.0">
            Licence: Creative Commons Attribution 4.0
        </a>
    </div>

        <img alt="Illustration representing open-source media" src="data/images/static/2015-02-09_philosophy_04-open-source.jpg">

    <h2>Kvalitní zábava pro všechny, ať už jsou kdekoliv</h2>

    <p>Pepper&amp;Carrot je zábavný, humorný komiks vhodný pro všechny, bez ohledu na věk.<br/>
    Žádné 18+, žádné násilí. Zdarma a open-source. Pepper&amp;Carrot je hrdým příkladem toho, jak skvělé mohou být i věci zadarmo.<br/>
    Hodně se zaměřuji na kvalitu, protože zdarma a open-source neznamená špatně a amatérsky. Právě naopak.</p>

        <img alt="" src="data/images/static/2015-02-09_philosophy_05-everyone.jpg">

    <h2>Společně změníme komiksový průmysl!</h2>

    <p>Bez prostředníka mezi umělcem a čtenáři, Vy zaplatíte méně a já víc získám. Podporujete totiž přímo mě.<br/>
    Žádný vydavatel/distributor/obchod/trend mě nemůže donutit změnit Pepper&amp;Carrot k jejich potřebám.<br/>
    &nbsp;.&nbsp;.&nbsp;.&nbsp;proč by nemohl jediný úspěch započít vlnu podobných projektů? Uvidíme...</p>
    

        <img alt="Illustration: comic book industry VS patreon support" src="data/images/static/2015-02-09_philosophy_06-industry-change.jpg">

    <div class="philobutton">
        <a class="h4" href="https://www.patreon.com/davidrevoy" title="Jen za 1$ na Patreonu podpořte novou epizodu Pepper&amp;Carrot">
            Pomozte mi pozvednout produkci Pepper&amp;Carrot ještě dnes.
        </a>
    </div>

    <p>Díky za přečtení! <br/>
    -David Revoy</p>

 ',

################################################################################
# Page: Contribute
'CONTRIBUTE_TITLE'		=> 'Podílet se',
'CONTRIBUTE_TOP'		=> '
    <p>Díky <a href="?static6/sources" title="Sources page">open source</a> a <a href="http://creativecommons.org/licenses/by/4.0/">
    licenci Creative commons </a> se můžete na Pepper&amp;Carrot podílet mnoha způsoby:</p>
',
'CONTRIBUTE_DONATION'		=> '
    <h2>Patreon</h2>

    <p>Všechno ohledně financování rozepisuji do detailu na <a href="?static2/philosophy" title="otevřít stránku Pojetí">stránce Pojetí</a>.<br/>
    Fanouškem Pepper&amp;Carrot se <a href="https://www.patreon.com/davidrevoy">na Patreonu</a> můžete stát už za 1 dollar na epizodu.<br/>
    Patreon přijímá všechny platební karty z celého světa a dokonce můžete platit i přes PayPal.<br/>
            Také mě můžete podpořit bezprostředně přes PayPal nebo Flattr:</p>
',
'CONTRIBUTE_FANART'	    => '
    <h2>Tvorba fanoušků</h2>
    <p>Pepper&amp;Carrot podporuje vlastní tvorbu svých fanoušků: kresby, scénáře, sošky, 3D modely, fanfiction. Pošlete mi je na (<a href="mailto:info@davidrevoy.com">info@davidrevoy.com</a>, nebo mi dejte vědět přes sociální sítě), a objeví se v galerii fanouškovských výtvorů:</p>
',
'CONTRIBUTE_DERIVATIONS' => '
    <h2>Adaptace:</h2>
    <p>Pepper&amp;Carrot může být přepracován do spousty dalších projektů a produktů, tak proč se k nějakému nepřipojit nebo dokonce nezačít vlastní?</p>
',

'CONTRIBUTE_BOTTOM'	    => '
    <h2>Překlady a korekce</h2>

    <p>Pepper&amp;Carrot je multijazyčný web, schopný přijmout jakýkoliv jazyk (včetně těch vymřelých nebo fiktivních). Zdroje potřebné k překladu jsou volně k dostání tady na stránkách. Pro víc informací si přečtěte <a href="index.php?fr/article267/translation-tutorial">návod pro překladatele</a>.</p>

    <h2>Tisk, sociální sítě</h2>

    <p>Staňte se vydavatelem Pepper&amp;Carrot! Pište články do novin nebo na weby, sdílejte a pomáhejte vytvořit komunitu na Vašich oblíbených sociálních sítích. <a href="?static6/sources" title="Sources page">Materiály pro tisk</a> si můžete stáhnout na stránce Zdrojů.</p>

    <h2>Další možnosti, jak se podílet&nbsp;.&nbsp;.&nbsp;.&nbsp;</h2>

    <p>Každý se může zapojit jinak:<br/>
        <b>Vývojáři:</b> Vytvořit aplikaci, která umožní čtení Pepper&amp;Carrot na mobilu.<br/>
        <b>Hudebníci:</b> Složit hudební motiv k Pepper&amp;Carrot.<br/>
        <b>Spisovatelé:</b> Navrhnout scénář nové epizody Pepper&amp;Carrot.<br/>
        <b>Novináři:</b> Napsat o Pepper&amp;Carrot v tradičních médiích (tištěné noviny, TV, atd.)<br/>
        <b>Tiskárny:</b> Vytisknout plakáty a další s motivem Pepper&amp;Carrot.<br/>
    </p>

    <h2>IRC kanál:</h2>

    <p>Chatujte a diskutujte o Pepper&amp;Carrot. Mě tam můžete zastihnout během dne v evropském času (přezdívka:deevad)<br/>
',

################################################################################
# Page: Sources
'SOURCES_TITLE'		    => 'Zdroje',
'SOURCES_TOP'	        => '
    <p><b>Vítejte v centru plném stahovatelných zdrojů!</b><br/>
    Najdete tady zdrojové soubory Pepper&amp;Carrot a mnohem víc.<br/>
    Všechny soubory s digitálními ilustracemi jsou kompatibilní s poslední verzí <a href="https://krita.org/">Kritu</a>.</p>
',
'SOURCES_BOTTOM'	    => '
    <p>Stáhnutím a prací s těmito soubory souhlasíte, že budete respektovat podmínky
    <a href="http://creativecommons.org/licenses/by/4.0/">licence Creative Commons Attribution</a>.
    Pro více informací si přečtěte soubor README, přiložený ke každému projektu.</p>

    <p><b><i class="fa fa-github" style="font-size:1.5em;" ></i>Úložiště překladů, scénářů, webovek, grafiky atd.:</b><br/></p>
',
'SOURCE_COVER'   =>  'Titulka',
'SOURCE_KRITA'   =>  'Zdrojové soubory ke Kritu',
'SOURCE_TRANSLATOR'=>  'Překladatelský balíček',
'SOURCE_WEB'      =>  'Web <span class="sourceinfos"> 90ppi, lehké jpg </span>',
'SOURCE_PRINT'        =>  'Tisk <span class="sourceinfos"> 300ppi, bez ztráty kvality </span>',
'SOURCE_MONTAGE'   =>  '<span class="sourceinfos"> Všechny panely uspořádané na jednu stranu </span>',

################################################################################
# Page: Author
'AUTHOR_TITLE'		=> 'O Davidu Revoyovi',
'AUTHOR_BIO'	    => '
    <p>Ahoj, jmenuju se David Revoy a jsem francouzský umělec narozený roku 1981. Jsem samouk a vášnivě zamilovaný do kreslení, malování, koček, počítačů, Gnu/Linux open-source kultury, internetu, RPG videoher ze staré školy, starých mang a anime, tradičního umění, fantasy&nbsp;.&nbsp;.&nbsp;.&nbsp;<br/>
    <br/>
    Po více než 10 letech nezávislého digitálního malování, učení, koncepční grafiky, ilustrací a výtvarné režie, jsem se rozhodl začít vlastní projekt. Konečně jsem přišel na způsob, jak spojit všechny moje záliby dohromady a výsledkem byl Pepper&amp;Carrot.</p>

    <p>Moje portfolio:</p>
',
'AUTHOR_TODO_DREAM'	=> '
    <h3>Seznam přání:</h3>

    <p>
    ☐ Mít projev o Pepper&amp;Carrot na japonském comic-conu.<br/>
    ☐ Zahrát si na gamepadu Pepper&amp;Carrot videohru.<br/>
    ☐ Mít v galerii 100 fan-artů.<br/>
    ☑ <a href="http://www.peppercarrot.com/en/article376/dream-to-do-list-a-wikipedia-page">Mít stránku na Wikipedii.</a><br/>
    ☐ Dostat fotku cosplaye Feferonky.<br/>
    ☑ <a href="http://www.peppercarrot.com/article304/dream-to-do-list-a-cat-named-carrot">Dostat fotku zrzavého kocoura pojmenovaného Karotka.</a><br/>
    ☑ <a href="http://www.peppercarrot.com/article302/peppercarrot-has-over-500-supporters">Mít podporu 500 fanoušků na Patreonu.</a><br/>
    ☐ Dosáhnout 100. epizody!<br/>
    </p>
',

'AUTHOR_CARREER_TITLE'				 => 'Moje kariéra v sedmi bublinách:',
'AUTHOR_CARREER_BUBBLE_DESCRIPTIONS' => '<p></p>',

################################################################################
# Page: Wiki
'WIKI'		=> 'Wiki',

################################################################################
# Website General: Footer
'FOOTER_CONTENT'    => '
    <p>Pokud není na stránce uvedeno jinak, všechny komiksy, ilustrace a texty spadají pod licenci <a href="https://creativecommons.org/licenses/by/4.0/" title="Pro více informací si přečtěte Creative Commons Attribution 4.0">Creative Commons Attribution 4.0</a>.<br/>
    Autor: "David Revoy, www.davidrevoy.com". Pro více informací, mě kontaktujte na: <a href="mailto:info@davidrevoy.com">info@davidrevoy.com</a></p>

    <p>Webová stránka poháněna <a href="http://www.pluxml.org" title="PlXml">PluXml</a></p>
',

################################################################################
# various utils:
'UTIL_NEXT_EPISODE'      =>  'další epizoda',
'UTIL_PREVIOUS_EPISODE'  =>  'předchozí epizoda',
'UTIL_EPISODE'   =>  'epizoda',
'UTIL_BACK_TO_GALLERY'      =>  'zpět do galerie',
'UTIL_MORE'      =>  'více',
'UTIL_PAGE'      =>  'strana',
'UTIL_BY'        =>  ':',
'UTIL_LICENSE'   =>  'licence',
'FIRST'        =>  'první',
'LAST'   =>  'poslední',
);

?>
